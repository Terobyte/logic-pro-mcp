...
public enum MMCCommands {
    /// Play: command 0x02
    public static func play(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
        sysEx(deviceID: deviceID, command: 0x02)
    }

    /// Stop: command 0x01
    public static func stop(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
        sysEx(deviceID: deviceID, command: 0x01)
    }

    /// Record Strobe (punch-in): command 0x06
    public static func recordStrobe(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
        sysEx(deviceID: deviceID, command: 0x06)
    }

    /// Record Exit (punch-out): command 0x07
    public static func recordExit(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
        sysEx(deviceID: deviceID, command: 0x07)
    }

    /// Pause: command 0x09
    public static func pause(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
        sysEx(deviceID: deviceID, command: 0x09)
    }

    /// Fast Forward: command 0x04
    public static func fastForward(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
        sysEx(deviceID: deviceID, command: 0x04)
    }

    /// Rewind: command 0x05
    public static func rewind(deviceID: UInt8 = MIDIConfig.mmcDeviceID) -> [UInt8] {
    ...
    public static func locate(
        ...
        frames: UInt8,
        subframes: UInt8 = 0,
        deviceID: UInt8 = MIDIConfig.mmcDeviceID
    ) -> [UInt8] {
        [
